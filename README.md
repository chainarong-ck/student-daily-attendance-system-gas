...
เตรียม Google Apps Script project, Google Sheet อย่างน้อย 1 ไฟล์ และ Node.js/npm สำหรับ build โปรเจกต์
...
- ดาวน์โหลด HTML ที่รักษารูปแบบ Header, Content, Footer และตาราง
...
| `getReportTemplates(token)` | app | โหลดเทมเพลตที่เปิดใช้งานของ current year |
| `getReportTemplatesForAcademicYear(adminToken, academicYearKey)` | admin | โหลดเทมเพลตจากปีการศึกษาที่เลือก |
| `saveReportTemplates(adminToken, rows)` | admin | บันทึกเทมเพลตของ current year |
